...
public interface RestaurantRepository extends JpaRepository<Restaurant, Integer>{
	...
	@Query(value="SELECT * FROM restaurant_detail r where r.state=:state",nativeQuery = true)
	Page<Restaurant> findByState(@Param("state") String state,Pageable pageable);
	
	@Query(value="select * from restaurant_detail res left join restaurant_diet die on res.id=die.restaurant_id where  res.state=:state and die.diet_id IN (select d.id from diets d where d.name in :dietType) group by res.id",nativeQuery = true)
	Page<Restaurant> findByDiets(@Param("state") String state,@Param("dietType") List<String> dietType,Pageable pageable);
	
	@Query(value="select * from restaurant_detail res left join restaurant_types typ on res.id=typ.restaurant_id where  res.state=:state and typ.types_id IN (select t.id from types t where t.name in :restaurantType) group by res.id",nativeQuery = true)
	...
	@Query(value="select * from restaurant_detail res left join restaurant_seatings sea on res.id=sea.restaurant_id where  res.state=:state and sea.seatings_id IN (select s.id from seatings s where s.name in :restaurantSeating) group by res.id",nativeQuery = true)
	...
	@Query(value="select * from restaurant_detail res left join restaurant_cuisines cus on res.id=cus.restaurant_id where  res.state=:state and cus.cuisines_id IN (select c.id from cuisines c where c.name in :restaurantCuisine) group by res.id",nativeQuery = true)
	...
	@Query(value="select * from restaurant_detail res left join restaurant_services ser on res.id=ser.restaurant_id where  res.state=:state and ser.services_id IN (select s.id from services s where s.name in :restaurantService) group by res.id",nativeQuery = true)
	...
	@Query(value = "SELECT res.* FROM restaurant_detail res JOIN( SELECT  ad.id, (6371 * acos (cos ( radians(:latitude) ) * cos( radians( ad.lattitude ) )* cos( radians( ad.longitude ) - radians(:longitude) ) + sin ( radians(:latitude) )* sin( radians( ad.lattitude ) ))) AS distance FROM address_detail ad ) AS T ON res.address_id = T.id LEFT JOIN restaurant_services ser ON res.id=ser.restaurant_id WHERE res.state= :state AND ser.services_id IN (SELECT s.id FROM services s WHERE s.name IN :restaurantService) GROUP BY res.id ORDER BY T.distance\r\n"
			+ "", nativeQuery = true)
	...
	@Query(value = "SELECT res.* FROM restaurant_detail res JOIN( SELECT  ad.id, (6371 * acos (cos ( radians(:latitude) ) * cos( radians( ad.lattitude ) )* cos( radians( ad.longitude ) - radians(:longitude) ) + sin ( radians(:latitude) )* sin( radians( ad.lattitude ) ))) AS distance FROM address_detail ad ) AS T ON res.address_id = T.id LEFT JOIN restaurant_cuisines cus ON res.id=cus.restaurant_id WHERE res.state= :state AND cus.cuisines_id IN (SELECT s.id FROM cuisines s WHERE s.name IN :restaurantCuisine) GROUP BY res.id ORDER BY T.distance", nativeQuery = true)
	...
	@Query(value = "SELECT res.* FROM restaurant_detail res JOIN( SELECT  ad.id, (6371 * acos (cos ( radians(:latitude) ) * cos( radians( ad.lattitude ) )* cos( radians( ad.longitude ) - radians(:longitude) ) + sin ( radians(:latitude) )* sin( radians( ad.lattitude ) ))) AS distance FROM address_detail ad ) AS T ON res.address_id = T.id LEFT JOIN restaurant_seatings sea ON res.id=sea.restaurant_id WHERE res.state= :state AND sea.seatings_id IN (SELECT s.id FROM seatings s WHERE s.name IN :restaurantSeating) GROUP BY res.id ORDER BY T.distance\r\n"
			+ "", nativeQuery = true)
	...
	@Query(value = "SELECT res.* FROM restaurant_detail res JOIN( SELECT  ad.id, (6371 * acos (cos ( radians(:latitude) ) * cos( radians( ad.lattitude ) )* cos( radians( ad.longitude ) - radians(:longitude) ) + sin ( radians(:latitude) )* sin( radians( ad.lattitude ) ))) AS distance FROM address_detail ad ) AS T ON res.address_id = T.id LEFT JOIN restaurant_types typ ON res.id=typ.restaurant_id WHERE res.state= :state AND typ.types_id IN (SELECT s.id FROM types s WHERE s.name IN :restaurantType) GROUP BY res.id ORDER BY T.distance\r\n"
			+ "", nativeQuery = true)
	...
	@Query(value = "SELECT res.* FROM skipthequeue.restaurant_detail res JOIN( SELECT  ad.id, (6371 * acos (cos ( radians(:latitude) ) * cos( radians( ad.lattitude ) )* cos( radians( ad.longitude ) - radians(:longitude) ) + sin ( radians(:latitude) )* sin( radians( ad.lattitude ) ))) AS distance FROM skipthequeue.address_detail ad ) AS T ON res.address_id = T.id LEFT JOIN skipthequeue.restaurant_diet die ON res.id=die.restaurant_id WHERE res.state= :state AND die.diet_id IN (SELECT s.id FROM skipthequeue.diets s WHERE s.name IN :dietType) GROUP BY res.id ORDER BY T.distance\r\n"
			+ "", nativeQuery = true)
