...
public interface RestaurantRepository extends JpaRepository<Restaurant, Integer> {
	...
	@Query(value = "SELECT * FROM restaurant_detail r LEFT JOIN timings tim ON tim.restaurant_id=r.id where tim.day like :day and r.state=:state", nativeQuery = true)
	Page<Restaurant> findByConfirmedState(@Param("state") String state, @Param("day") String day, Pageable pageable);

	@Query(value = "SELECT * FROM restaurant_detail r where r.state=:state and r.restaurant_name like %:restaurantName%", nativeQuery = true)
	Page<Restaurant> findByName(@Param("state") String state, @Param("restaurantName") String restaurantName,
			 Pageable pageable);

	@Query(value = "SELECT * FROM restaurant_detail res LEFT JOIN(SELECT  ad.id, (6371 * acos (cos ( radians(:latitude) ) * cos( radians( ad.lattitude ) )* cos( radians( ad.longitude ) - radians(:longitude) ) + sin ( radians(:latitude) )* sin( radians( ad.lattitude ) ))) AS distance FROM address_detail ad) AS T ON res.address_id = T.id  LEFT JOIN timings tim ON tim.restaurant_id=res.id where tim.day like :day and res.state=:state and T.distance < :km ORDER BY T.distance", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM restaurant_detail res LEFT JOIN(SELECT  ad.id, (6371 * acos (cos ( radians(:latitude) ) * cos( radians( ad.lattitude ) )* cos( radians( ad.longitude ) - radians(:longitude) ) + sin ( radians(:latitude) )* sin( radians( ad.lattitude ) ))) AS distance FROM address_detail ad) AS T ON res.address_id = T.id LEFT JOIN timings tim ON tim.restaurant_id=res.id where tim.day like :day AND res.state=:state ORDER BY T.distance", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM restaurant_detail res LEFT JOIN(SELECT  ad.id, (6371 * acos (cos ( radians(:latitude) ) * cos( radians( ad.lattitude ) )* cos( radians( ad.longitude ) - radians(:longitude) ) + sin ( radians(:latitude) )* sin( radians( ad.lattitude ) ))) AS distance FROM address_detail ad ) AS T ON res.address_id = T.id LEFT JOIN restaurant_services ser ON res.id=ser.restaurant_id LEFT JOIN restaurant_cuisines cus ON res.id=cus.restaurant_id LEFT JOIN restaurant_seatings sea ON res.id=sea.restaurant_id LEFT JOIN restaurant_diet die ON res.id=die.restaurant_id LEFT JOIN restaurant_types typ ON res.id=typ.restaurant_id LEFT JOIN timings tim ON tim.restaurant_id=res.id WHERE tim.day like :day AND res.state=:state AND ser.services_id IN (SELECT s.id FROM services s WHERE s.name IN :restaurantService ) AND seatings_id IN (SELECT s.id FROM seatings s WHERE s.name IN :restaurantSeating ) AND cus.cuisines_id IN (SELECT s.id FROM cuisines s WHERE s.name IN :restaurantCuisine ) AND typ.types_id IN (SELECT s.id FROM types s WHERE s.name IN :restaurantType ) AND die.diet_id IN (SELECT s.id FROM diets s WHERE s.name IN :restaurantDiet ) GROUP BY res.id ORDER BY T.distance", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM restaurant_detail res LEFT JOIN(SELECT  ad.id,(6371 * acos (cos ( radians(:latitude) ) * cos( radians( ad.lattitude ) )* cos( radians( ad.longitude ) - radians(:longitude) ) + sin ( radians(:latitude) )* sin( radians( ad.lattitude ) ))) AS distance FROM address_detail ad) AS T ON res.address_id = T.id  LEFT JOIN promotion_restaurant pr ON res.id=pr.restaurant_id  LEFT JOIN timings tim ON tim.restaurant_id=res.id where res.state like :state and  tim.day like :day and T.distance < :km ORDER BY pr.rank_id,pr.id LIMIT 5", nativeQuery = true)
